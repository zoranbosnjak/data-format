...
{-# OPTIONS_GHC -Wall -Wno-unticked-promoted-constructors -Wno-missing-signatures #-}
...
type Schema = RawSchema FieldName StrLength -- type-level
type PlainContent = RawContent PlainFieldName PlainStrLength
type PlainSchema = RawSchema PlainFieldName PlainStrLength -- term-level
...
--
-- However, for statically indexing the schema, a list-like structure is easiest.
--
...
data TField (c :: Content) where
  TFixed    :: String -> TField (StrFixed n) -- TODO: using String here is wrong; should be a fixed-length entity
...
parseAnySchema (SDBCons is iss) = TInDB Z <$> parseIndexed is <|> (\ (TInDB i x) -> (TInDB (S i) x)) <$> parseAnySchema iss

parseIndexed :: SIndexedSchema is -> Parser (TIndexed is)
parseIndexed (SIndexed schema0) = go schema0
  where
    -- the helper function is used to get access to the type n, which unfortunately we
...
parseSchema (SCons (SField SFixed) schema0)     = go schema0
  where
    -- the helper function is used to get access to the type n, which unfortunately we
    -- cannot pattern-match on directly ...
    go :: forall fn n s . (KnownSymbol fn, KnownNat n) => SSchema s -> Parser (TValue ('(fn, StrFixed n) : s))
    go schema = do
...
renderSchema (SCons (SField SFixed) schema0) v0 = go schema0 v0
  where
    go :: forall fn n s' . (KnownSymbol fn, KnownNat n) => SSchema s' -> TValue ('(fn, StrFixed n) : s') -> String
    go schema (TCons (TFixed x) v) = x <> renderSchema schema v
renderSchema (SCons (SField SVariable) schema0) v0 = go schema0 v0
  where
    go :: forall fn s' . KnownSymbol fn => SSchema s' -> TValue ('(fn, StrVariable) : s') -> String
    go schema (TCons (TVariable x) v) = show (length x) <> x <> renderSchema schema v
renderSchema (SCons (SField (SNested s0)) schema0) v0 = go s0 schema0 v0
  where
    go :: forall fn s' s'' . KnownSymbol fn => SSchema s'' -> SSchema s' -> TValue ('(fn, Nested s'') : s') -> String
    go s schema (TCons (TNested x) v) = renderSchema s x <> renderSchema schema v
...
accessorExample :: String
accessorExample =
  let
    schema = SIndexed @2 sschema2
    Right x = parse (parseIndexed schema) "" exampleChunk3
  in
    renderIndexed schema (over (indexed . access @"C" . access @"E") ("ff" ++) x)
